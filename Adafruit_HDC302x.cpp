...
Adafruit_HDC302x::Adafruit_HDC302x() {
  currentAutoMode = EXIT_AUTO_MODE;
}
 ...
 */
hdcAutoMode_t Adafruit_HDC302x::getAutoMode() const {
 ...
 * @return true if the data was successfully read and CRC checks passed, otherwise false.
 */
...
/**
 * Reads the temperature and humidity on demand using the specified trigger mode.
 * 
 * @param temp Reference to store the temperature value.
 * @param RH Reference to store the relative humidity value.
 * @param mode The trigger mode to use for the measurement, defaults to TRIGGERMODE_LP0 (lowest noise)
 * @return true if the data was successfully read and CRC checks passed, otherwise false.
 */
bool Adafruit_HDC302x::readTemperatureHumidityOnDemand(double &temp, double &RH, 
                                                       hdcTriggerMode_t mode=TRIGGERMODE_LP0) {
 ...
 * @return true if the data was successfully read and CRC checks passed, otherwise false.
 */
bool Adafruit_HDC302x::sendCommandReadTRH(uint16_t command, double &temp, double &RH) {
...
bool Adafruit_HDC302x::readOffsets(double &T, double &RH) {
    ...
    if (!writeCommandReadData(HDC302x_Commands::ACCESS_OFFSETS, combined_offsets)) {
        return false;
    }
...
/**
 * Calculates the closest matching offset byte for temperature or relative humidity.
 * 
 * @param f The floating point offset value.
 * @param isTemp True if calculating temperature offset, false if calculating RH offset.
 ...
 * @param isTemp True if inverting temperature offset, false if inverting RH offset.
 ...
 * @return true if all parts of the NIST ID were read successfully and CRC checks were valid, otherwise false.
 */
bool Adafruit_HDC302x::readNISTID(uint8_t nist[6]) {
    uint16_t part1, part2, part3;
    if (!writeCommandReadData(HDC302x_Commands::READ_NIST_ID_SERIAL_BYTES_5_4, part1) ||
        !writeCommandReadData(HDC302x_Commands::READ_NIST_ID_SERIAL_BYTES_3_2, part2) ||
        !writeCommandReadData(HDC302x_Commands::READ_NIST_ID_SERIAL_BYTES_1_0, part3)) {
        return false;
    }
 ...
 * @return true if the command and data transmission were successful and CRC checks out.
 */
